# Write a program that checks if a list is already sorted. For a list to 
# be sorted, the next element must NOT be smaller than the previous one.
...
for _ in range(input_lines):
    ...
    for i in range(len(new_line) - 1):
        if int(new_line[i]) > int(new_line[i + 1]):
            is_ascending = False
            break
